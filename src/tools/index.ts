...
export class PremiereProTools {
  ...
  getAvailableTools(): MCPTool[] {
    return [
      ...
      {
        name: 'reframe_for_916',
        description: 'Reframes a 16:9 clip for 9:16 vertical video by applying calculated scale and position transforms.',
        inputSchema: z.object({
          clipId: z.string().describe('The node ID of the clip to reframe'),
          framePosition: z.enum(['center', 'top', 'bottom', 'left', 'right']).optional().describe('Where to position the frame (default: center)')
        })
      },
  ...
  private async reframeFor916(
    clipId: string,
    framePosition: 'center' | 'top' | 'bottom' | 'left' | 'right' = 'center'
  ): Promise<any> {
    // 16:9 -> 9:16 reframe: scale 177.78% to fill, position to taste
    const scale = 177.78;
    let position: [number, number];

    switch (framePosition) {
      case 'top':    position = [0.5, 0.3]; break;
      case 'bottom': position = [0.5, 0.7]; break;
      case 'left':   position = [0.3, 0.5]; break;
      case 'right':  position = [0.7, 0.5]; break;
      case 'center':
      default:       position = [0.5, 0.5]; break;
    }
